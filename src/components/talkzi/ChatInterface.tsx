...
export function ChatInterface() {
  ...
  const handleSendMessage = useCallback(async (userInput: string) => {
    ...
    try {
      ...
      const companionInput: HinglishAICompanionInput = { 
        message: messageForAI,
        history: chatMemory.chatHistory.slice(-MAX_CONTEXT_MESSAGES).map(h => ({
          role: 'user',
  ...
  }, [user, profile, currentAiFriendType, chatMemory, addMessage, updateChatMemory, incrementChatCount]);
